...
This script builds a vector database from the examples in examples-annotate.json
and examples-classify.json using SBERT embeddings for similarity search.
...
class VectorDatabaseBuilder:
    ...
    def load_examples(self, examples_dir: Path) -> None:
        """
        Load examples from both JSON files.
        ...
        annotate_file = examples_dir / "examples-annotate.json"
        if annotate_file.exists():
            with open(annotate_file, 'r', encoding='utf-8') as f:
                self.examples_data['annotate'] = json.load(f)
        ...
        classify_file = examples_dir / "examples-classify.json"
        if classify_file.exists():
            with open(classify_file, 'r', encoding='utf-8') as f:
                self.examples_data['classify'] = json.load(f)
        ...
        for example_type in ['annotate', 'classify']:
            ...
            for i, example in enumerate(self.examples_data[example_type]):
                ...
                metadata = {
                    ...
                    'source_file': f"examples-{example_type}.json",
